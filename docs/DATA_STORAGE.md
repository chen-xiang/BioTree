...
本文约定分类数据、多语言内容与配图的存储方式。与 [TECH_STACK.md](./TECH_STACK.md)、[PERFORMANCE.md](./PERFORMANCE.md)、[FULL_TAXONOMY_PLAN.md](./FULL_TAXONOMY_PLAN.md) 配套。

---

## 1. 数据范围

- 目标覆盖权威源中的 **完整阶元体系**（含中间级）；产品默认浏览可投影为 **界 / 门 / 纲 / 目 / 科 / 属 / 种** 七级视图（见完整阶元方案）。
- 覆盖 Catalogue of Life 七界（动物、古菌、细菌、色藻、真菌、植物、原生生物）；可用 `app.import.kingdoms` 缩小范围。
- 节点需支持 **详细介绍**（多语言）及 **配图**。
